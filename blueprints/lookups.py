...
@bp.route(route="lookups/types-vw", methods=["GET"])
def get_lookup_types_view(req: func.HttpRequest) -> func.HttpResponse:
    """Get lookup types from pdc_lookup_types_vw view."""
    with get_db() as db:
        try:
            lookup_service = PDCLookupService(db)
            items = lookup_service.get_lookup_types_view()
            return create_success_response({"items": items})
        except Exception as e:
            logging.error(f"Get lookup types view failed: {str(e)}")
            return create_error_response("Failed to retrieve lookup types view", 500, str(e))


@bp.route(route="lookups/codes-vw", methods=["GET"])
def get_lookup_codes_view(req: func.HttpRequest) -> func.HttpResponse:
    """Get lookup codes from pdc_lookup_codes_vw view."""
    with get_db() as db:
        try:
            lookup_service = PDCLookupService(db)
            lookup_type = None
            # Support lookup_type passed in body (optional) or query param
            try:
                body = req.get_json()
                if isinstance(body, dict):
                    lookup_type = body.get("lookup_type")
            except Exception:
                pass
            if not lookup_type:
                lookup_type = req.params.get("lookup_type")

            items = lookup_service.get_lookup_codes_view(lookup_type=lookup_type)
            return create_success_response({"items": items})
        except Exception as e:
            logging.error(f"Get lookup codes view failed: {str(e)}")
            return create_error_response("Failed to retrieve lookup codes view", 500, str(e))
...
def get_lookup_type(req: func.HttpRequest) -> func.HttpResponse:
    ...
    with get_db() as db:
        try:
            ...
            lookup_service = PDCLookupService(db)
            # Prefer view for lookup type (pdc_lookup_types_vw). Fallback to base table.
            db_lookup_type = lookup_service.get_lookup_type_view(lookup_type)
            if db_lookup_type:
                response_data = db_lookup_type.to_dict()
                return create_success_response(response_data)
